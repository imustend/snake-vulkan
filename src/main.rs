...
use image::{ImageBuffer, Rgba, image_dimensions};
use vulkano::{
...
fn main() {
    ...
    */
    ...
    mod cs {
        vulkano_shaders::shader!{
            ty: "compute",
            path: "./mandelbrot.comp",
        }
    }

    

    let shader = cs::load(device.clone()).expect("could not load shader");
    ...
    let buff = CpuAccessibleBuffer::from_iter(
        device.clone(),
        BufferUsage::all(),
        false,
        (0..1024 * 1024 * 4).map(|_| 0u8),
    ...
    builder
        ...
        .unwrap()
        .copy_image_to_buffer(image.clone(), buff.clone())
        .unwrap();
    ...
    let buffer_content = buff.read().unwrap();
    let image = ImageBuffer::<Rgba<u8>, _>::from_raw(1024, 1024, &buffer_content[..]).unwrap();
